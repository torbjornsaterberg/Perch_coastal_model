...
sensitivity_models_fit_files <- grep("fit_", sensitivity_models_all_files, value = TRUE) # paths to mcmc summary files
...
baseline_models_fit_files <- grep("fit_", baseline_models_all_files, value = TRUE) # paths to saved files
baseline_models_inputdata_files <- grep("inputdata_", baseline_models_all_files, value = TRUE) # paths to saved files
...
for(i in c(1:length(sensitivity_models_fit_files))) {
  fit <- readRDS(sensitivity_models_fit_files[i])
  input <- readRDS(sensitivity_models_inputdata_files[i])
  fit_summary <- restruct_STAN_fit(fit, input)
  Lokal <- sub(".*fit_([^_]+)_.*", "\\1", sensitivity_models_fit_files[i])
  Depth_stratum <- sub(".*_([^_]+)\\.RDS$", "\\1", sensitivity_models_fit_files[i])
  ...
  fit <- readRDS(baseline_models_fit_files[i])
  input <- readRDS(baseline_models_inputdata_files[i])
  fit_summary <- restruct_STAN_fit(fit, input)
  Lokal <- sub("fit_(.*)\\d{4}-\\d{2}-\\d{2}.*", "\\1", basename(baseline_models_fit_files[i]))
...
# create figure  
  ...
  geom_smooth(method="lm", se=FALSE) +  
    #  geom_bar(stat="identity", fill=Depth_stratum, position = "dodge") +
...
for (i in 1:length(Areas)) {
  ...
  labs(
    x = "Year",
    y = expression(paste("Z"['a, t']," (mean mortality)"))
  ) +
  scale_color_manual(values = pal) +
...
# save MCMC data
fit$save_object(paste0("../MCMCOutput/fit_Råneå", Sys.Date(),".RDS"))
```
